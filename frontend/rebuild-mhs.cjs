...
const suratList = ref([]);
const suratTab = ref('draft'); // 'draft' or 'complete'
...
const saveSurat = async (isFinal = false) => {
  try {
    ...
    if(data.success) {
      ...
      fetchSurat();
      if(isFinal) {
        // Trigger PDF generation (optional, can be done manually)
      }
    } else {
...
`;
  ...
  let fbHtml = adminContent.substring(fbStart, fbEnd).replace(/adminToken/g, 'token');
  let ctxHtml = adminContent.substring(fbEnd, ctxEnd).replace(/adminToken/g, 'token');
...
`;
  ...
  console.log('Mahasiswa Dashboard FULLY REBUILT with SURAT and EXPLORER!');
